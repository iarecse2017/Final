...
function id_showFBC(src) {
	...
	if(src == 0) {
		document.getElementById('fbIframeNT').src = "https://www.intensedebate.com/fb-connect/fbConnect.php?acctid=" + commentObj.acctid + "&token=" + commentObj.token;
		showFBLoginNewThread();
	} else {
		document.getElementById('fbIframeR').src = "https://www.intensedebate.com/fb-connect/fbConnect.php?acctid=" + commentObj.acctid + "&token=" + commentObj.token;
		showFBLoginReply();
	}
...
function id_fbPoll() {
	...
	id_fbPollScript.src='https://www.intensedebate.com/fb-connect/getFB.php?acctid=' + commentObj.acctid + '&token=' + commentObj.token + '&time=' + d.getTime();
...
function postComment(src)
{
	...
	if(src==0) //new thread
	{
		if( commentObj.postUsingTwitter ) {
			...
			IDReplaceHtml($id("IDNewThreadSubmitLI"), '<img src="https://www.intensedebate.com/images/ajax-loader.gif" alt="Submitting..." />');
	...
	else
	{
		if( commentObj.postUsingTwitter ) {
			...
			IDReplaceHtml($id("IDReplyDivSubmitLI"), '<img src="https://www.intensedebate.com/images/ajax-loader.gif" alt="Submitting..." />');
			var requestObj = new buildRequestObj(theStr, 0, null, connectionErr);
			xs.make_request(requestObj);
...
function showForgotPassword() {
	showMsgBox( "Forgot your password?", "<form><label for=\"txtResolveEmail\">Please enter your email:</label><input type=\"text\" class=\"idc-text\" id=\"txtResolveEmail\" style=\"color:black\" onfocus=\"txtOnFocus(this, '');\" onblur=\"txtOnBlur(this, '');\" /><a href=\"javascript: forgotPassword();\" class=\"idc-btn_s\"><span></span><span class='idc-r'>Send password reset link</span></a></form>", 0 );
...
function showWPLogin() {
	$id("IDCommentsHead").className = $id("IDCommentsHead").className.replace(/idc-signup/g, "").replace(/idc-openid/g, "").replace(/idc-openid_signup/g, "").replace(/idc-wp-login/g, "");
	var wpccWin = window.open( "https://intensedebate.com/logmein?account_type=WPCC&idc-auth", null, 'location=0,status=0,width=1000,height=600, left=200,top=100' );
	var wpccMonitor = setInterval( function() {
		if ( !wpccWin || wpccWin.closed ) {
			// Reload when we're done
			clearInterval( wpccMonitor );
...
function collapseThread(commentid) {
	if($id("IDCommentCollapseLink" + commentid) && $id("IDCommentCollapseLink" + commentid).className == "idc-collapselink") {
		...
		if( $id("IDCommentSubThread" + commentid).childNodes.length == 0 ) { //content not loaded yet
			...
			IDReplaceHtml($id("IDCommentSubThread" + commentid), "<div class='idc-thread_loading'><img src='https://www.intensedebate.com/images/ajax-loader.gif' alt='Loading...' /> Loading...</div>");
...
function showFollowThisDiscussion()
{
	...
	{
		$id('IDCommentsShowNetvibes').src    = 'https://www.netvibes.com/img/add2netvibes.gif';
		$id('IDCommentsShowYahoo').src       = 'https://s.yimg.com/lq/i/us/my/addtomyyahoo4.gif';
		$id('IDCommentsShowGoogle').src      = 'https://i0.wp.com/buttons.googlesyndication.com/fusion/add.gif';
		$id('IDCommentsShowMsn').src         = 'https://tkfiles.storage.msn.com/x1pHd9OYNP16fmmfqJHji7qY0yYomKrFzGROBps3O6qHF0JRlVV8xH6X4cfsptw0fftk5oJYFpTKP6I-i91-se8TaoO7R9oiPVoxDEG_LEZW_XhegHxASvHJYsSxNjf526t';
		$id('IDCommentsShowRss').src         = 'https://www.intensedebate.com/themes/chameleon/images/feed-icon-12x12.png';
		$id('IDCommentsShowNetvibes2').src   = 'https://www.netvibes.com/img/add2netvibes.gif';
		$id('IDCommentsShowYahoo2').src      = 'https://s.yimg.com/lq/i/us/my/addtomyyahoo4.gif';
		$id('IDCommentsShowGoogle2').src     = 'https://i0.wp.com/buttons.googlesyndication.com/fusion/add.gif';
		$id('IDCommentsShowMsn2').src        = 'https://tkfiles.storage.msn.com/x1pHd9OYNP16fmmfqJHji7qY0yYomKrFzGROBps3O6qHF0JRlVV8xH6X4cfsptw0fftk5oJYFpTKP6I-i91-se8TaoO7R9oiPVoxDEG_LEZW_XhegHxASvHJYsSxNjf526t';
		$id('IDCommentsShowRss2').src        = 'https://www.intensedebate.com/themes/chameleon/images/feed-icon-12x12.png';
	}
};
...
function IDsaveComment(commentid)
{
	...
	IDReplaceHtml($id("IDCommentCancelSave"+commentid), '<img src="https://www.intensedebate.com/images/ajax-loader.gif" alt="Submitting..." />');
...
function IDCSubscribeByEmail(type)
{
	...
	IDReplaceHtml($id('IDCSubscribeSubmit'+type), '<img src="https://www.intensedebate.com/images/ajax-loader.gif" alt="Submitting..." />');
...
IDC.user_menu = {
	...
	requestObj.service_url = 'https://www.intensedebate.com/js/getUserMenu.php';
	xs.make_request(requestObj);
    },
    ...
    "loading_div" : IDC.c_object( 'div', {'id':'idc-usermenu-loading', 'innerHTML': '<div class="idc-m-avatar idc-m-avatar-loading"><img class="idc-m-loadingimg" src="https://www.intensedebate.com/themes/universal/images/idc-m-loading.gif" alt="Loading..."/></div>' })
};

IDC.user_menu.clear_cache();
if(commentObj) commentObj.anon_name = 'sravan'; if(document.getElementById('txtNameNewThread')) document.getElementById('txtNameNewThread').value = 'sravan'; if(document.getElementById('txtNameReply')) document.getElementById('txtNameReply').value = 'sravan';if(commentObj) commentObj.anon_email = 'mrsrsvan549@gmail.com'; if(document.getElementById('txtEmailNewThread')) document.getElementById('txtEmailNewThread').value = 'mrsrsvan549@gmail.com'; if(document.getElementById('txtEmailReply')) document.getElementById('txtEmailReply').value = 'mrsrsvan549@gmail.com';var IDAdminIsLoggedIn=false;var argList = new Array();
argList['userid'] = '';
